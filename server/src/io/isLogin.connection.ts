...
export default (socket: any) => {
  ...
  socket.on('isLogin', async (user: any) => {
    if (!global._.isEmpty(user)) {
      ...
      const signature = await getWxConfigUtil.calcSignature(
        wxTicket,
        nonceStr,
        timestamp,
      ...
      console.log(nonceStr)  
      console.log(timestamp)
      console.log(`${domain}${socket.request.url}`)
      // 将微信服务的签名返回到前端展示
      // console.log(`here is isLogin.conn 23 page ${userInfo}`)
      // 返回去的token码是不带socket值的
      // 返回用户信息好签名
      socket.emit(`isLogin_${emit}`, {userInfo , wxConfig: {appId, timestamp, nonceStr, signature}});
    }
  });
};
