<template>
	<v-dialog v-model="showModal" :showHead="true" :showFoot="true" :showHeadBg="true" :closeBack="modelCloseBack">
		...
		<div data-page="user-authentication" slot="body">
			<div class="mui-content mui-input-group">
				<div class="mui-input-row" style="height:auto;">
					...
					<button @click="sheetToggle" class="upload-button up-btn"><i class="iconfont icon-shouchishenfenzheng" style="margin-right: 5px;"></i>上传照片</button>
          <div class="id-photo">
            <i class="iconfont icon-morentupian_da" v-if="defaultIdPhoto === ''"></i>
            <img :src="defaultIdPhoto" v-else>
          </div>
				</div>
        <div class="mui-input-row">
        <label>身份证</label>
    			<input type="text" class="mui-input-clear" placeholder="请输入证件号">
    		</div>
        <div class="mui-input-row">
          <label style="width:auto;">银行卡号</label>
          <input type="text" class="mui-input-clear" style="width: calc(100% - 87px);">
        </div>
        <div class="mui-input-row">
          <label>手机号</label>
          <input type="text" class="mui-input-clear" placeholder="绑定银行卡手机号">
        </div>  
			</div>
		</div>
		<div slot="footer" class="view-foot">
        <button type="button" class="mui-btn mui-btn-primary" data-loading-icon="mui-spinner" data-loading-icon-position="right" data-loading-text="提交中..." >提交认证</button>
		</div>
	</v-dialog>
</template>
<script>
