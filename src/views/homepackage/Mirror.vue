<template>
    <div  class="mirror" ref="mirror">
    	<div class="outer-circle around" :style="around" ref="around">
        <figure style="width: 160vw;position: fixed;">    
          <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0" y="0" viewBox="0 0 136 166" xml:space="preserve">              
              ...
              <use id="sea" xlink:href="#circ"></use>
              <g id="boat1" class="vehicles">
                <use xlink:href="#boat"></use>
              </g>
              <g id="land">
                <use xlink:href="#map" transform="translate(300,-309)"></use>
                <use xlink:href="#map" transform="translate(600,-309)"></use>
              </g>
              <rect id="hi" mask="url(#light)" width="300" height="300"></rect>
              <g class="vehicles">
                <g id="plane1">
        ...
        <rollers :Points='Points' :TweezerOnSet='TweezerOnSet' :Name="Name" ></rollers>
    </div>
</template>
<script>
...
export default {
  ...
  methods: {
    $_mirror_putPoints() {
      let pointX = window.innerWidth,
        pointY = this.$refs.mirror.offsetHeight / 2,
        pointR = this.$refs.around.offsetWidth + 20,
        menus = Array.from(require('../../js/data/menus.json'))
      return this.$_mirror_getPoint(pointR, pointX, pointY, menus)
    },
    /*
    * 求圆周上等分点的坐标
    * ox,oy为圆心坐标
    ...
    * count为等分个数
    */
    $_mirror_getPoint(r, ox, oy, menus) {
      let point = [], //结果
        src = '', //弧度
        i = 1,
        avd = 360 / menus.length

      for (; i <= menus.length; i++) {
        let x = Math.cos(avd * i * Math.PI / 180) * r + (ox - 32.5),
          y = Math.sin(avd * i * Math.PI / 180) * r + (oy - 57),
          deg = avd * i,
          src = require('../../imgs/NR/' + i + '.png')
        if (deg == 200) {
          this.TweezerOnSet = { x, y }
          this.Name = menus[i - 1].name
        }
        point.unshift({ x, y, src, deg, menus: menus[i - 1] }) //为保持数据顺时针
      }
      return point
    }
  }
...
.mirror {
  ...
  //   position: absolute;
  //   box-shadow: -20px 0 0 rgba(139, 157, 169, 0.28);
  //   top: 0;
  //   right: 0px;
  // }
  // .outer-circle:after {
  ...
  //   border-bottom-left-radius: 0;
  //   position: absolute;
  //   box-shadow: 20px -1px 0px rgba(139, 157, 169, 0.28);
  //   top: calc(~'50% - 1px');
