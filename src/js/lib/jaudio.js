...
(function($){
  ...
  Plugin.prototype = {
    ...
    events: function()
    {
      ...
      // - controls events
      self.$domControls.off().on("click", "button", function()
      {
      ...
      // - playlist events
      self.$domPlaylist.off('click.jAudio--playlist-item').on("click.jAudio--playlist-item", ".jAudio--playlist-item", function(e)
      {
        var item = $(this),
      ...
      self.$domPlaylist.off('click.join-song').on("click", ".join-song", function(e)
      {
        /**
         * 先去实现加入歌单功能
         * 
      ...
      // 点击加入歌单
      self.$domPlaylist.off('click.share-song').on("click", ".share-song", function(e)
      {
         // 点击分享
      /**
       * 分享先不去实现
      ...
      // - progress bar events
      self.$domProgressBar.off().on("click", function(e)
      {
        self.updateProgressBar(e);
        self.updateElapsedTime();
      })

      $(self.domAudio).off().on("loadedmetadata", function()
      {
    ...
    renderPlaylist: function()
    {
      ...
      $.each(self.settings.playlist, function(i, a)
      {
        var file          = a["file"],
            musicId =  a["musicId"],
            isInPlay_list =  a["isInPlay_list"] || false,
            thumb         = a["thumb"],
            trackName     = a["trackName"],
            trackArtist   = a["trackArtist"],
            trackAlbum    = a["trackAlbum"];
          //  trackDuration = "00:00";
        var isInPlay_list_class = isInPlay_list ? ' icon-chuangjiangedan':' icon-jiarugedan'
        template += "<div class='jAudio--playlist-item' data-track='" + file + "'>";
        template += "<input class='inp-value' value='"+musicId+"' type='hidden'/>";
        ...
        template += "<p class='track-art-ist'>" + trackArtist + "</p>";
        template += "</div>";
        // template += "<div class='jAudio--playlist-track-duration'>" + trackDuration + "</div>";
        template += "<div class='jAudio--playlist-track-buttons'>";
        // 分享 加入歌单
        template += "<div class='share-song'><i class='iconfont icon-wode-fenxiang'></i></div>";
        template += "<div class='join-song'><i class='iconfont "+isInPlay_list_class+"'></i></div>";
        template += "</div>";
        template += "</div>";
...
export default jQuery(".jAudio--player").jAudio;